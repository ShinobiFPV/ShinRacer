...
module.exports = function createInvitesRouter(io) {
  ...
  router.post('/', (req, res) => {
    try {
      const { serverName, host, port, httpPort, password, track, cars, expiresIn, created_by } = req.body
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          httpPort: invite.http_port, password: invite.password, track: invite.track,
          cars: invite.cars, expiresIn,
        },
      })
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message })
