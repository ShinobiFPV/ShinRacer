...
// Horizontal or vertical drag slider. `value` is runtime state (owned by
// ClusterRuntime) unless telemetryBind is set, in which case it's a
// read-only display of the live telemetry value (dragging is disabled).
export default function SliderWidget({ config = {}, mode, telemetryFrame, value, onValueChange }) {
  ...
  function drag(clientX, clientY) {
    ...
    onValueChange?.(valueFromEvent(clientX, clientY))
  }
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, padding: 8 }}>
      ...
      <div
        ...
        style={{
          ...
          width: horizontal ? '100%' : 10, height: horizontal ? 10 : '100%', flex: horizontal ? 'none' : 1,
        }}
      >
        <div style={{
        ...
        <div style={{
          position: 'absolute', width: 16, height: 16, borderRadius: '50%', background: cfg.thumbColor,
