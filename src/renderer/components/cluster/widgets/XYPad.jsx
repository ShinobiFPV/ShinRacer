...
// 2D touch/drag area — X and Y axes fire independently via onValueChange
// with { x, y } each normalized 0-1 (0,0 is top-left). `value` is runtime
// state owned by ClusterRuntime, same pattern as the other input widgets.
export default function XYPad({ config = {}, mode, value, onValueChange }) {
  ...
  function fromEvent(clientX, clientY) {
  ...
  function drag(clientX, clientY) {
    ...
    onValueChange?.(fromEvent(clientX, clientY))
  }
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      <div
        ...
        style={{ position: 'relative', flex: 1, background: cfg.backgroundColor, border: `1px solid ${C.border}`, cursor: isRuntime(mode) ? 'crosshair' : 'default', overflow: 'hidden' }}
      >
        {cfg.gridLines && (
        ...
        <div style={{
          ...
          width: 14, height: 14, borderRadius: '50%', border: `2px solid ${cfg.crosshairColor}`, background: `${cfg.crosshairColor}33`,
        }} />
      </div>
    </div>
  )
