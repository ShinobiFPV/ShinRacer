...
function StepShell({ emoji, name, subtitle, children }) {
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <div style={{ fontFamily: C.head, fontWeight: 700, fontSize: 28, marginBottom: 6 }}>{name}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        {SESSION_TYPES.map(t => (
          <button key={t.id} onClick={() => onSelect(t.id)}
            style={{ background: value === t.id ? `${C.yellow}12` : C.raised,
              border: `1px solid ${value === t.id ? C.yellow : C.border}`,
            ...
            <div style={{ fontFamily: C.head, fontWeight: 700, fontSize: 16, marginBottom: 4, color: C.white }}>{t.label}</div>
          ...
          <div style={{ maxHeight: 280, overflowY: 'auto', marginTop: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
            {filtered.map(t => (
              <button key={t.name} onClick={() => onSelect(t)}
                style={{ background: value?.name === t.name ? `${C.yellow}12` : C.raised,
                  border: `1px solid ${value?.name === t.name ? C.yellow : C.border}`, borderRadius: 6,
                  padding: '10px 14px', textAlign: 'left', display: 'flex', justifyContent: 'space-between',
                  alignItems: 'center', cursor: 'pointer' }}>
                <span style={{ fontFamily: C.mono, fontSize: 12, color: C.white }}>{t.name}</span>
          ...
          <div style={{ maxHeight: 260, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
            {filtered.map(car => (
              <Tooltip key={car} text="Select which car models players can choose from">
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderRadius: 5,
                  background: selected.includes(car) ? `${C.yellow}12` : C.raised,
                  border: `1px solid ${selected.includes(car) ? C.yellowDim : C.border}`, cursor: 'pointer' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 20 }}>
        {WEATHER_OPTIONS.map(w => (
          <button key={w.value} onClick={() => onWeather(w.value)}
            style={{ background: weather === w.value ? `${C.yellow}12` : C.raised,
              border: `1px solid ${weather === w.value ? C.yellow : C.border}`, borderRadius: 7,
              padding: '12px 8px', textAlign: 'center', cursor: 'pointer' }}>
            <div style={{ fontSize: 20, marginBottom: 4 }}>{w.icon}</div>
            <div style={{ fontSize: 11, color: C.white }}>{w.label}</div>
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
        {TIME_OPTIONS.map(t => (
          <button key={t.value} onClick={() => onTime(t.value)}
            style={{ background: t.gradient, border: `2px solid ${timeOfDay === t.value ? C.yellow : 'transparent'}`,
              borderRadius: 7, padding: '14px 8px', textAlign: 'center', cursor: 'pointer', color: '#fff' }}>
    ...
    <StepShell emoji="📋" name="Rules & limits" subtitle={subtitle}>
      <div style={{ marginBottom: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
          ...
          <span style={{ fontFamily: C.mono, fontSize: 12, color: C.yellow }}>{driverCountLabel(maxClients)} ({maxClients})</span>
        </div>
  ...
  return (
    <StepShell emoji="🏁" name="Launch it" subtitle={summary}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 22 }}>
        {cells.map(([label, val]) => (
          <div key={label} style={{ background: C.raised, border: `1px solid ${C.border}`, borderRadius: 6, padding: '10px 8px', textAlign: 'center' }}>
            <div style={{ fontFamily: C.mono, fontSize: 13, fontWeight: 700, color: C.yellow }}>{val}</div>
    ...
    <div style={{ position: 'fixed', inset: 0, background: C.bg, zIndex: 50, animation: 'fadeUp .2s ease',
      ...
      <div style={{ padding: '20px 28px 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <div style={{ fontFamily: C.head, fontWeight: 700, fontSize: 14, color: C.mutedHi, textTransform: 'uppercase', letterSpacing: 0.6 }}>
            {STEP_NAMES[step - 1]}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <span style={{ fontFamily: C.mono, fontSize: 11, color: C.muted }}>Step {step} of {TOTAL_STEPS}</span>
            <button onClick={onClose} style={{ background: 'none', border: 'none', color: C.muted, fontSize: 20, cursor: 'pointer' }}>✕</button>
          </div>
        </div>
        <div style={{ height: 4, background: C.border, borderRadius: 2, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${(step / TOTAL_STEPS) * 100}%`, background: C.yellow, transition: 'width .25s ease' }} />
        </div>
      </div>
