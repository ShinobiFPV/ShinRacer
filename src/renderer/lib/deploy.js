...
export async function deployConfig(cfg, settings) {
  ...
  const result = await api.server.launch({
    ...
    // name/port/httpPort/password were previously never sent to the main process at all,
    // which silently broke player-count polling (it needs httpPort) — now required by the
    // Content Manager wrapper too, so fixed here rather than duplicated at both call sites.
    name: cfg.name, port: cfg.port, httpPort: cfg.httpPort,
    password: cfg.password, adminPassword: settings.adminPassword,
    cmWrapper: cfg.cmWrapper,
  })
