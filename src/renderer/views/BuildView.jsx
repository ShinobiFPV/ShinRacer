...
export function defaultCfg(serverName) {
  return {
    ...
    cmWrapper: { enabled: false, port: 9080, description: '' },
  }
}
...
export default function BuildView({ initialCfg, onDeployed, onOpenWizard }) {
  ...
  const setCM = (k, v) => setCfg(prev => ({
    ...prev,
    cmWrapper: { enabled: false, port: 9080, description: '', ...prev.cmWrapper, [k]: v },
  }))
        ...
        <Card style={{ flex: 1 }}>
          ...
          {tab === 'sessions' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <Tooltip text="Lets friends browsing in Content Manager see a rich listing for this server — description, assist rules, and a password check — instead of the bare name AC's own server list shows">
                <Toggle label="Content Manager wrapper" value={cfg.cmWrapper?.enabled || false} onChange={v => setCM('enabled', v)}
                  hint="Runs a small local proxy alongside acServer.exe" />
              </Tooltip>
              {cfg.cmWrapper?.enabled && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 12, marginLeft: 4 }}>
                  <div>
                    <Label>Wrapper port</Label>
                    <TextInput value={cfg.cmWrapper.port} onChange={v => setCM('port', +v)} mono
                      placeholder="must differ from HTTP port" />
                  </div>
                  <div>
                    <Label>Description shown in Content Manager</Label>
                    <TextInput value={cfg.cmWrapper.description} onChange={v => setCM('description', v)}
                      placeholder="optional — e.g. welcome message or rules" />
                  </div>
                </div>
              )}
            </div>
