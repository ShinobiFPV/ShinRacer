...
- Real Content Manager integration — a wrapper toggle gives your server a rich CM listing (description, assist rules, a password check) instead of the bare name AC's own server list shows
- Share a server and get back a real join link — friends click straight into Content Manager's join flow, no typing host:port by hand
- UPnP opens your router automatically for a direct-internet join when your network allows it; behind carrier-grade NAT, it tells you straight instead of pretending it worked
- "Kill all servers" when old test runs pile up and you just want a clean slate
...
**On Tailscale?** Open `https://your-pi.tail9249a1.ts.net:8443`, sign in,
...
**Prefer a real installed app over "Add to Home Screen"?** There's also an
Android APK — same PWA, wrapped as a Trusted Web Activity, so it opens
full-screen with zero browser chrome and shows up as a real app, not a
bookmark. On Tailscale, grab it at
`https://your-pi.tail9249a1.ts.net:8443/shinracer.apk` (Android will ask to
allow installs from Chrome the first time — expected). Details, rebuild
steps, and the signing-key setup: **[docs/ANDROID_TWA_SETUP.md](docs/ANDROID_TWA_SETUP.md)**.
...
The heavy lifting runs on a Pi 5 in your-pi's setup. Everyone else just runs the app.
...
**The backend** is the one thing that has to be shared — a small always-on Node service holding the events calendar, chat history, WebRTC signaling, lap stats, host registrations, roles, and invite codes in a single SQLite database, pushing realtime updates over Socket.io. It lives on your-pi's Pi 5 as a systemd service, but there's nothing Pi-specific about it — any always-on box on the network does the job. The same Pi also serves the mobile PWA over nginx (not pictured above, for diagram simplicity) — same backend, separate codebase, separate deploy.
...
| Networking | Tailscale (or LAN), hand-rolled UPnP client for opportunistic direct-internet hosting |
...
Roles live in `backend/config/roles.json` on your-pi (not in git, not in the
