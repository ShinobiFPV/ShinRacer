...
configured in `backend/.env` on your-pi.

## 2. `roles.json` — the permission source of truth

Roles live in `backend/config/roles.json` on your-pi — **not** in the
...
2. Find that account's Google UID. Easiest path: on your-pi, check
   `backend/ac_companion.db`'s `users` table (every account that's ever
   signed in gets a row there) —
   ```bash
   sqlite3 /home/your-pi/ac-companion-backend/ac_companion.db \
     "SELECT uid, email, name FROM users;"
   ```
3. Copy `backend/config/roles.json.example` to
   `backend/config/roles.json` on your-pi (if it doesn't exist yet) and add
...
- **System Health** — the backend process's own uptime/memory, plus a
  **Restart backend** button (with a confirm step) that runs
  `sudo systemctl restart ac-companion` on your-pi. Passwordless sudo for
  exactly that command is already configured on your-pi (see
