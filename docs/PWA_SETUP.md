...
The PWA is served by nginx on `your-pi`, reverse-proxying to the existing
...
2. Open **`https://your-pi.tail9249a1.ts.net:8443`** in the phone's browser
   (Safari on iOS, Chrome on Android) — the HTTPS Tailscale Serve URL, not
   the plain-HTTP `192.168.1.100:8080` one. Google sign-in needs a secure
...
- Visiting `https://your-pi.tail9249a1.ts.net:8443/` loads the ShinRacer
