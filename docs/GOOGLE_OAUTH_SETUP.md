...
4. Under **Authorized redirect URIs**, add:
   ```
   https://shinobi.tail9249a1.ts.net:8443/auth/callback
   ```
...
Add both to `backend/.env` on shinobi:

```
GOOGLE_OAUTH_CLIENT_ID_PWA=<the Client ID from step 5>
GOOGLE_OAUTH_CLIENT_SECRET_PWA=<the Client Secret from step 5>
GOOGLE_OAUTH_REDIRECT_URI_PWA=https://shinobi.tail9249a1.ts.net:8443/auth/callback
```
...
```bash
curl -s https://shinobi.tail9249a1.ts.net:8443/api/auth/config
```
...
shinobi — reverse-proxies into the same nginx `:8080` block, Tailscale
...
(imq2/Q2, proxying to port 80) — check `tailscale serve status` on shinobi
