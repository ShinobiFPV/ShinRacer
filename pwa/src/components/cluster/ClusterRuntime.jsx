...
import { getWidgetEntry } from './widgets'

const STATEFUL_WIDGET_TYPES = ['toggleButton', 'rotaryEncoder', 'slider', 'xyPad']

// Ported from the Electron app's src/renderer/components/cluster/ClusterRuntime.jsx —
// identical runtime logic (no edit-mode branches needed here at all, since
// the PWA never runs the editor). See that file's comments for why the
// runtime state map exists.
export default function ClusterRuntime({ layout, telemetryFrame, onAction }) {
  ...
  function setValue(widgetId, value) { setRuntimeState(prev => ({ ...prev, [widgetId]: value })) }
  ...
  return (
    <div style={{
      ...
      {[...(layout.widgets || [])].sort((a, b) => (a.zIndex ?? 0) - (b.zIndex ?? 0)).map(w => {
        ...
        const cfg = w.config || {}
        const common = { key: w.id, config: cfg, mode: 'runtime', telemetryFrame }
