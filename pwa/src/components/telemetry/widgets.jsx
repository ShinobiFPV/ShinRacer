...
// Ported from the Electron app's src/renderer/components/telemetry/widgets.jsx
// (Phase 9) — pure React/SVG with no Electron dependency, so it's a faithful
// copy rather than a reimplementation. Kept as a literal duplicate across the
// pwa/ ↔ src/ build boundary for the same reason colors.js/presetLinks.js
// are (Phase 10) — the two apps build and deploy independently. Update both
// if the gauge visuals ever change.
...
export function SpeedGauge({ frame, config = {} }) {
  ...
  const pct = Math.max(0, Math.min(1, speedKmh / 300))
  ...
  return (
    <div style={{ width: 160, height: 160, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <svg width={160} height={160} style={{ position: 'absolute', inset: 0 }}>
        ...
        {pct > 0 && <path d={describeArc(cx, cy, r, startAngle, fillAngle)} fill="none" stroke={C.blue} strokeWidth={4} />}
      </svg>
